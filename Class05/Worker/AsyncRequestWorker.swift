...
class AsyncRequestWorker {
    ...
    func getResponse(from:String, tag:Int) -> Void {
        ...
        let task = session.dataTask(with: request, completionHandler: { (fetchData, response, error) in
            ...
            if( 200 == statusCode){
                ...
                let str = String(decoding: fetchData!, as: UTF8.self)
                
                self.reponseDelegate?.receviedReponse(self, responseString: str, tag: tag)
            }
